...
export default class ABI {
  ...
  static rawEncode (types, values): Buffer {
    ...
    for (let i = 0; i < types.length; i++) {
      ...
      var value = values[i]
      var cur: Buffer = encodeSingle(type, value)
...
function encodeSingle (type, arg) {
  ...
  if (type === 'address') {
    arg = arg.replace(/^(41)/, '0x'); // trx 特殊处理
    const numBuffer: Buffer = encodeSingle('uint160', parseNumber(arg))
    numBuffer.writeInt8(0x41, 11)
    return numBuffer
  } else if (type === 'bool') {
...
function decodeSingle (parsedType, data: Buffer, offset) {
  ...
  if (parsedType.name === 'address') {
    data.writeInt8(0x00, 11) // trx 特殊处理
    return `41` + decodeSingle(parsedType.rawType, data, offset).toArrayLike(Buffer, 'be', 20).toString('hex')
